...
export const metadata = {
    title: 'Internkoordinator | TrAMS',
    description: 'Internkoordinator i TrAMS - Trondheim Akuttmedisinske Studentforening',
}

export default async function InternalCoordinatorPage() {
    const internalCoordinator = await getBoardMemberByRole('internalCoordinator')

    if (!internalCoordinator) {
        notFound()
    }

    const profileImageUrl = internalCoordinator.profileImage
        ? urlFor(internalCoordinator.profileImage).url()
        : null

    const personalImageUrl = internalCoordinator.PersonalImage
        ? urlFor(internalCoordinator.PersonalImage).url()
        : null
                ...
                <Flex direction={{ base: 'column', md: 'row' }} gap={12} alignItems="flex-start">
                        ...
                        {personalImageUrl ? (
                            <Box width="90%" borderRadius="lg" overflow="hidden" >
                                <Image
                                    ...
                                    alt={`${internalCoordinator.name} personal`}
                                    width={300}
                                    height={300}
                    ...
                    <Box flex={1}>
                        <Heading as="h3" fontSize="2.5rem" mb={6}>
                            {internalCoordinator.name.toUpperCase()}
                        </Heading>

                        <Box className="portable-text" mb={8}>
                            <PortableText value={internalCoordinator.bio} components={portableTextComponents} />
                        </Box>
                        ...
                        <Box>
                            ...
                            <Text fontSize="1.1rem">
                                ...
                                <a
                                    href={`mailto:${internalCoordinator.email}`}
                                    style={{ color: 'var(--color-primary)', textDecoration: 'underline', fontWeight: 600 }}
                                >
                                    {internalCoordinator.email}
                                </a>
                            </Text>
                        </Box>
                    </Box>
